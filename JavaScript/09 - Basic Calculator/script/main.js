...
for (let i = 0; i < buttonsEl.length; i++) {
    buttonsEl[i].addEventListener("click", () => {
        ...
        if (buttonValue === "C") {
            ...
            caculateResult(); // "=" 버튼 클릭 시 계산 실행 (Calculate result on "=" button click)
        } else {
...
// 계산 실행 함수 (caculate result function)
function caculateResult() {
    try {
        inputFieldEl.value = eval(inputFieldEl.value); // 입력된 수식 계산 (Evaluate expression in input field)
    } catch (e) {
        // 계산 오류 처리 (Handle caculation errors)
        console.error("계산오류 : ", e);
        inputFieldEl.value = "오류"; // 오류 메시지 표시 (Display error message)
    }
}

// 값 추가 필드 (Append value function)
function appendValue(buttonValue) {
    inputFieldEl.value += buttonValue; // 기존 값에 새로운 값 연결 (Append new value to exsiting value)
}